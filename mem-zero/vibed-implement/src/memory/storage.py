...
class FileStorage:
    ...
    def clear_all(self):
        """Clear all conversation contexts (reset to empty state).
        
        This removes all in-memory data and deletes the storage file.
        Use this to start fresh for testing.
        """
        logger.info("🧹 Clearing all conversation contexts...")
        self.contexts = {}
        
        # Delete file if it exists
        if self.contexts_file.exists():
            self.contexts_file.unlink()
            logger.info(f"   Deleted: {self.contexts_file}")
